...
// Receipt means the app server accepted a tool output, never that mail was read.
// Persist accepted generations so a lost channel ACK cannot enqueue duplicates.
export function deliverer(binding, rpc) {
  ...
  return frame => {
    const work = inFlight.then(async () => {
      ...
      const thread = await loadedThread(rpc, binding);
      ...
      await rpc.request('turn/start', { threadId: binding.threadId, input: [],
        toolOutput: { name: 'khala_doorbell', namespace: 'khala', output: doorbell({ ...binding, root: ROOT }, snapshot) } });
...
export async function serveBinding(binding, signal) {
  const rpc = new Rpc(binding.socket);
  const instance = randomUUID();
  let server, socketPath, registered = false;
  try {
    await rpc.initialize();
    await loadedThread(rpc, binding);
    const version = rpc.serverInfo.userAgent.match(/^[^/\s]+\/([A-Za-z0-9._:+-]+)(?:\s|$)/)?.[1];
    ...
    server = channelServer(deliverer(binding, rpc));
    ...
    while (!signal.aborted && readBinding(binding)?.token === binding.token) {
      ...
      if (!signal.aborted) await loadedThread(rpc, binding);
    }
  } finally {
